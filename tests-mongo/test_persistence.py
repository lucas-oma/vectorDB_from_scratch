...
class TestPersistence:
    ...
    async def test_ivf_persistence_across_restart(self, client):
        ...
        # Test that IVF index works immediately after restart (auto-rebuild feature)
        # Our auto-rebuild feature should make IVF search work without manual retraining
        search_data = {
            "embedding": search_embedding,
            "k": 3
        }
        
        # Search should work immediately after restart due to auto-rebuild
        resp = await client.post(_url(f"/libraries/{library_id}/search"), json=search_data)
        _assert_status(resp, 200, "search after restart (auto-rebuild)")
        
        search_results_after = resp.json()
        assert len(search_results_after["results"]) > 0
        
        # Verify the results are similar to before restart (data persistence)
        # Note: IVF is approximate, so we check that we get results, not exact matches
        print(f"✅ IVF search returned {len(search_results_after['results'])} results after restart")
        
        # Optional: Test that manual retraining still works
        print("🔄 Testing manual retraining capability...")
        training_vectors = [chunk["embedding"] for chunk in chunks_after]
        train_data = {"sample_vectors": training_vectors}
        
        resp = await client.post(_url(f"/libraries/{library_id}/index/train"), json=train_data)
        _assert_status(resp, 202, "manual retrain IVF index")
        
        # Search should still work after manual retraining
        resp = await client.post(_url(f"/libraries/{library_id}/search"), json=search_data)
        _assert_status(resp, 200, "search after manual retraining")
        
        search_results_manual = resp.json()
        assert len(search_results_manual["results"]) > 0
        
        print("✅ IVF data persistence test passed!")

    @pytest.mark.asyncio
    async def test_lsh_simhash_persistence_across_restart(self, client):
        """Test that LSH SimHash index data persists when containers are restarted."""
        
        # Step 1: Create LSH SimHash library
        create_data = {
            "name": "LSH SimHash Persistence Test Library",
            "dims": 1024,
            "index_type": "lsh_simhash",
            "metadata": {"description": "Test LSH SimHash library for persistence"}
        }
        ...
        # Step 2: Create document and chunks
        document_data = {"title": "LSH SimHash Persistence Test Document", "metadata": {}}
        resp = await client.post(_url(f"/libraries/{library_id}/documents"), json=document_data)
        _assert_status(resp, 201, "create document")
        document_id = resp.json()["id"]
        ...
        # Step 3: Test search before restart
        search_embedding = generate_embedding("LSH SimHash persistence test")
        search_data = {
            "embedding": search_embedding,
            "k": 3
        }
        ...
        print("🔄 Restarting containers to test LSH SimHash persistence...")
        ...
        # Step 5: Verify LSH SimHash data still exists and works
        print("🔍 Verifying LSH SimHash data persistence...")
        
        # Check library still exists
        resp = await client.get(_url(f"/libraries/{library_id}"))
        _assert_status(resp, 200, "get LSH SimHash library after restart")
        
        library_after = resp.json()
        assert library_after["id"] == library_id
        assert library_after["index_type"] == "lsh_simhash"
        ...
        # Test that LSH SimHash index works after restart
        # LSH SimHash indexes should auto-rebuild like flat indexes
        search_data = {
            "embedding": search_embedding,
            "k": 3
        }
        
        # This should work because LSH SimHash auto-rebuilds from MongoDB data
        resp = await client.post(_url(f"/libraries/{library_id}/search"), json=search_data)
        _assert_status(resp, 200, "search after restart")
        ...
        print("✅ LSH SimHash data persistence test passed!") 